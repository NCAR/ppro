...
!   - TCWA2: Tsai et al. (analytical gamma distribution based)
!
! Authors:
...
contains
  ...
  subroutine ppro_init_coefs(coef_path, operator_name)
    ...
    if (current_operator == OPERATOR_ZHANG21) then
      ...
      write(*,'(A)') 'No coefficient files needed (analytical formulation)'
      write(*,'(A)') '=========================================='
    endif
  ...
  subroutine ppro_set_operator(operator_name)
    ...
    select case (trim(operator_name))
      ...
      case ('TCWA2', 'tcwa2', 'Tcwa2')
        ...
        write(*,*) 'PPRO: Selected TCWA2 operator (analytical formulation)'
      case default
